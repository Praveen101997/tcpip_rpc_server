...
def mv_file(user_id, cloud_dir_path,path2):
    path_valid, path_exists, rel_path_str = path_check(user_id, cloud_dir_path)

    dir = str(root_dir)+'/'+str(user_id)+'/'+str(rel_path_str)
    dir2 = os.path.dirname(dir)
    filename = os.path.basename(dir)
    dir3 = dir2+'/'+path2+'/'+filename

    if shutil.move(dir,dir3):
        with ServerProxy(name_server_url, allow_none=True) as name_proxy:
            saved = name_proxy.save_file_info([generate_file_info(args.server_id, dir3, filename)])
        with ServerProxy(name_server_url, allow_none=True) as name_proxy:
...
def cp_same_folder(user_id, cloud_dir_path,path2):
    path_valid, path_exists, rel_path_str = path_check(user_id, cloud_dir_path)
    ...
    if shutil.copy(dir,dir3):
        with ServerProxy(name_server_url, allow_none=True) as name_proxy:
            saved = name_proxy.save_file_info([generate_file_info(args.server_id, dir3, path2)])
        return True
    else:
        return False
...
def make_dirs(user_id, cloud_dir_path):
    ...
    if file.exists():
        ...
        file = open(path, 'w+')
        file.write("I am a sample file")
        file.close()
        with ServerProxy(name_server_url, allow_none=True) as proxy:
            files_registered = proxy.save_file_info([generate_file_info(args.server_id, path, 'temp.dat')])
...
class Handler(SimpleXMLRPCRequestHandler):
    def do_HEAD(s):
        ...
        s.end_headers()
        output = ""
        output += "<html><head><title>Server Running</title></head>"
        output += "<body><h1>Server is Running Successfully.</h1>"
        output += "</body></html>"
        s.wfile.write(output.encode(encoding='utf_8'))
    def do_GET(s):
...
if __name__ == '__main__':
    root_dir = Path.home() / 'xmlrpc_server_files'
